...
class PartnerCreateViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.url = reverse('partnerships:partners:media:update', kwargs={'partner_pk': cls.partner.pk})
    ...
    def test_get_other_view_as_gf(self):
        self.client.force_login(self.user_gf)
        response = self.client.get(self.url, follow=True)
        self.assertTemplateNotUsed(response, 'partnerships/partner_media_update.html')
        self.assertTemplateUsed(response, 'registration/login.html')

    def test_get_own_view_as_gf(self):
        self.client.force_login(self.user_gf)
        url = reverse('partnerships:partners:update', kwargs={'pk': self.partner_gf.pk})
        response = self.client.get(url)
        self.assertTemplateUsed(response, 'partnerships/partner_media_update.html')

    def test_get_faculty_view_as_gf(self):
