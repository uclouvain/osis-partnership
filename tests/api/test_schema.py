...
class ApiSchemaTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        if not options.args or options.args[0].split('.')[0] != 'partnership':
            raise SkipTest("Not testing partnership directly, do not test schema")
        super().setUpClass()
    ...
    def test_api_schema_matches_generation(self):
        with NamedTemporaryFile(mode='w+') as temp:
            ...
            call_command(
                'spectacular',
                urlconf='partnership.api.url_v1',
                generator_class='partnership.api.schema.PartnershipSchemaGenerator',
                file=temp.name,
            )
            if not filecmp.cmp('partnership/schema.yml', temp.name, shallow=False):
                with open('partnership/schema.yml', 'r') as schema_file:
                    schema_lines = schema_file.readlines()
                with open(temp.name, 'r') as new_schema_file:
                    new_schema_lines = new_schema_file.readlines()
                self.fail("Schema has not been re-generated:\n\n" + ''.join(difflib.unified_diff(
                    schema_lines,
                    new_schema_lines,
                    fromfile='schema.yml',
